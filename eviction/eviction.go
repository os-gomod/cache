...
// Evictor is implemented by every eviction policy.
...
// New constructs the Evictor for the requested policy.
func New(policy config.EvictionPolicy, maxBytes int64) Evictor {
	...
	default: // EvictLRU and unknown
		return newLRU(maxBytes)
	}
}

// ============================================================================
// mapSliceEvictor — base for LIFO, Random, TinyLFU
// ============================================================================
...
func evictFrontKeys(
	...
	for i := 0; i < count && order.Len() > 0; i++ {
		...
		key := front.Value.(string) //nolint:forcetypeassert // FIFO/MRU lists only store string keys
		order.Remove(front)
		delete(items, key)
...
func (e *lruEvictor) OnAdd(key string, entry *Entry) {
	...
	if el, ok := e.items[key]; ok {
		e.order.MoveToFront(el)
		el.Value.(*lruEntry).entry = entry //nolint:forcetypeassert // LRU list elements always hold *lruEntry
	} else {
...
func (e *lruEvictor) Evict(count int) []string {
	...
	for i := 0; i < count && e.order.Len() > 0; i++ {
		...
		le := back.Value.(*lruEntry) //nolint:forcetypeassert // LRU list elements always hold *lruEntry
		e.order.Remove(back)
...
func (h *lfuHeap) Push(
	...
	item := x.(*lfuItem) //nolint:forcetypeassert // heap only pushes *lfuItem
	item.index = len(*h)
	*h = append(*h, item)
}
...
func (e *lfuEvictor) Evict(count int) []string {
	...
	for i := 0; i < count && e.h.Len() > 0; i++ {
		item := heap.Pop(&e.h).(*lfuItem) //nolint:forcetypeassert // LFU heap entries are always *lfuItem
...
func (e *fifoEvictor) OnAccess(_ string, _ *Entry) {} // FIFO ignores accesses
...
func newRandom(maxBytes int64) *randomEvictor {
	return &randomEvictor{
		...
		rng:             rand.New(rand.NewPCG(0, 0)), //nolint:gosec // non-cryptographic
	}
}
...
// ============================================================================
// TinyLFU (W-TinyLFU approximation using frequency-based random eviction)
// ============================================================================
...
func newTinyLFU(maxBytes int64) *tinyLFUEvictor {
	return &tinyLFUEvictor{
		...
		rng: rand.New(
			rand.NewPCG(1, 0),
		), //nolint:gosec // eviction randomness is non-cryptographic
		maxBytes: maxBytes,
	}
}
...
// Evict picks the lowest-frequency candidate from a random sample of 5.
