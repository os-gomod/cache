...
// =============================================================================
// Main Demo - E-commerce Platform Cache Layer
// =============================================================================
...
func runResilienceDemo(
	...
	retryConfig := resilience.RetryConfig{
		...
		RetryableErr: func(err error) bool {
			return _errors.IsTimeout(err) || _errors.IsConnectionError(err)
		},
	}
...
func runErrorHandlingDemo(ctx context.Context, memCache *memory.Cache) {
	...
	cbError := _errors.CircuitOpenError("api.call")
	if _errors.IsCircuitOpen(cbError) {
		fmt.Println("  ✅ CircuitOpen error correctly identified")
	}

	rlError := _errors.RateLimitedError("api.call")
	if _errors.IsRateLimited(rlError) {
		fmt.Println("  ✅ RateLimited error correctly identified")
	}

	customErr := _errors.New("payment.process", "order-123", fmt.Errorf("insufficient funds")).
		WithMetadata("attempt", 3).
		WithMetadata("payment_method", "credit_card")

	if code := _errors.CodeOf(customErr); code == _errors.CodeInvalid {
		fmt.Printf("  ✅ Error with metadata: %s (code=%v)\n", customErr.Error(), code)
	}

	if metadata, ok := _errors.GetMetadata(customErr, "attempt"); ok {
...
func main() {
	...
	// Get the original product for CAS demo
