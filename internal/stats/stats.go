// Package stats provides atomic, lock-free cache statistics.
//
// Concurrency model:
//   - All counters use sync/atomic for lock-free reads and writes.
//   - Reset() is NOT globally atomic: a Snapshot() taken concurrently may
//     observe a partially-reset state.  Replace the cache instance when a
//     perfectly clean baseline is required.
//   - Snapshot() reads each counter independently; values are individually
//     consistent but not collectively consistent across a single instant.
package stats
...
// Stats tracks cache operations using lock-free atomic counters.
// The zero value is valid; prefer NewStats() to also record a start time.
type Stats struct {
	hits      int64
	misses    int64
	gets      int64
...
// NewStats creates a Stats instance with the start time set to now.
func NewStats() *Stats {
	s := &Stats{}
...
//revive:disable-next-line:get-return // GetOp records a get operation; it is not a getter.
func (s *Stats) GetOp()      { inc(&s.gets) }
...
// Snapshot is an immutable point-in-time copy of Stats values.
...
// TakeSnapshot returns an immutable copy of current stats.
...
// Snapshot is a deprecated alias for TakeSnapshot.
//
// Deprecated: Use TakeSnapshot.
func (s *Stats) Snapshot() Snapshot { return s.TakeSnapshot() }

// Reset zeroes all counters and resets the start time to now.
func (s *Stats) Reset() {
