...
// NewMemoryManager creates a CacheManager with a single in-memory backend
// registered as the default. It uses DefaultPolicy unless overridden
// with additional Option values.
...
// NewRedisManager creates a CacheManager with a single Redis backend
// registered as the default. It uses DefaultPolicy unless overridden
// with additional Option values.
...
// NewLayeredManager creates a CacheManager with a layered (L1 memory + L2
// redis) backend registered as the default. The L1 and L2 backends are
// constructed from the provided options. It uses DefaultPolicy unless
// overridden with additional Option values.
